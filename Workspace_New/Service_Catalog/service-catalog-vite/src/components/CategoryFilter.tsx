...
const CategoryFilter = ({
    ...
    const handleCheckboxChange = (filterType: string, value: string) => {
        switch (filterType) {
            case 'brand':
                onFilterChange(
                    'brands',
                    selectedBrands.includes(value) ? selectedBrands.filter((b) => b !== value) : [...selectedBrands, value]
                );
                break;
            case 'material':
                onFilterChange(
            ...
            case 'goldKarat':
                onFilterChange(
                    'goldKarats',
                    selectedGoldKarats.includes(value)
            ...
            case 'size':
                onFilterChange(
                    'sizes',
                    selectedSizes.includes(value) ? selectedSizes.filter((s) => s !== value) : [...selectedSizes, value]
                );
                break;
            case 'color':
                onFilterChange(
                    'colors',
                    selectedColors.includes(value) ? selectedColors.filter((c) => c !== value) : [...selectedColors, value]
                );
                break;
        }
    };
    ...
    const handlePriceChange = (min: string, max: string) => {
        const minValue = min === '' ? 0 : parseInt(min);
        const maxValue = max === '' ? 100000000 : parseInt(max);
    ...
    return (
        <div className="bg-white rounded-lg shadow p-4">
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={onResetFilters} className="text-blue-600 hover:text-blue-800 text-sm">
                    Xóa bộ lọc
                </button>
            </div>
                ...
                {expandedSections.price && (
                    <div className="pl-2">
                        ...
                        <div className="flex justify-between text-xs text-gray-600 mt-1">
                            ...
                            <span>
                                {priceRange[1] === 100000000 ? 'Không giới hạn' : priceRange[1].toLocaleString('vi-VN')}
                            </span>
                        </div>
                    </div>
                )}
